...
let subirPdf = async (req, res) => {
  ...
  if (codUser === undefined) {
    let datos = [
      { idUsuario: idUser, codigo_documento: codDoc, path: pathPdf },
    ];
    console.log(datos);
    await db("documentos")
      .insert(datos)
...
let getPdf = (req, res) => {
  ...
  db.select("*")
    .from("documentos")
    .where("codigo_documento", "like", cond)
...
updatePdf = async (req, res) => {
  ...
  await db
    ...
    .then((registro) => {
      ...
      db("documentos")
        .where("codigo_documento", cod)
        .update({ path: pathPdf, fechaModificacion: fecha })
        .then((resultado) => {
          return res.status(200).json({
            txt: true,
            data: resultado,
